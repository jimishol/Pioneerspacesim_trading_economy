...
local function updateEquipmentStock (station)
	...
	for key, e in pairs(Equipment.cargo) do
		if e.purchasable then
			local rn = math.min(StockOne / (math.abs(e.price))^PriceExp, 10^6)--have less than 1,000,000 stock per commodity. On low prices rn are large integers that can not exceed much the 2*10^9 anyway.
			if e == hydrogen then
