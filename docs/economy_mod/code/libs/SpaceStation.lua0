...
local function updateEquipmentStock (station)
	...
	for systemp,arrtime,deptime in FlightLog.GetSystemPaths() do
		...
		if time_interval < 60 * 30 * 24 * 60 * 60 and Game.system.name == systemp:GetStarSystem().name then
	...
	for key, e in pairs(Equipment.cargo) do
		if e.purchasable then
			local rn = math.min(10^9 / (math.abs(e.price))^2.42328282947206, 10^6)--have less than 1,000,000 stock per commodity. On low prices rn are large integers that can not exceed much the 2*10^9 anyway.
			if e == hydrogen then
				...
				elseif pricemod < -10 then --major export
					stock = stock +(recentSystem and rn or (rn * 0.6)) 
				elseif pricemod < -4 then --minor export
					stock = stock + (recentSystem and (rn*0.2) or (rn*0.4))
				end
...
-- Adding prerequisite feature to ships in order to exclude them from function addRandomShipAdvert if player is not their prequisite. Prerequisite is table for example prerequisites["Sinonatrix"] = {"Amphiesma", "Lunar Shuttle"}

local function prerequisite(pship,xship)
	local prerequisites ={}
...
local function addRandomShipAdvert(station, num)
	...
	for i=1,num do
		...
		local avail = utils.build_array(utils.filter(function (k,def) return (playersShip == def.name or prerequisite(playersShip,def.name)) end, pairs(avail_0)))
