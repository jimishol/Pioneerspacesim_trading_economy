...
local makeAdvert = function(station, commodity)
	...
	-- Amount should not exceed what station normally wants (relevant values at line 57 -former line 48-- of SpaceStation.lua). Actually we set to desperately want the 0.25 of his normal needs at double its price. ad.price/2 because it is already doubled. Comment disabling the use of lograndom function.
	--local money_to_spend = lograndom(1e2, math.min(10^9 / (math.abs(ad.price))^1.42328282947206, 10^6))
	local maxMoney = math.max(math.abs(ad.price)+1.00, math.min(StockOne / (math.abs(ad.price)/2)^(PriceExp - 1)*1.3/2))
